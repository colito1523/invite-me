...
export const loadExistingStories = async (t, setStories, setUnseenStories, isUploading) => {
  try {
...
export const uploadStory = async (imageUri, t, setIsUploading, setUploadProgress, setStories, setUnseenStories) => {
  try {
    const user = auth.currentUser
    if (!user) {
...
export const compressImage = async (uri, quality = 0.6, width = 1080) => {
  try {
    const manipulatedImage = await ImageManipulator.manipulateAsync(uri, [{ resize: { width } }], {
      compress: quality,
