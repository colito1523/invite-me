...
const triggerCrop = () => {
  if (cropTimeout) clearTimeout(cropTimeout);
...
export default function EditablePhoto({ uri, index, onSave }) {
    ...
    // Usamos un ref para almacenar el timeout
    const cropTimeoutRef = React.useRef(null);
  
    // Definición de triggerCrop dentro del componente
    const triggerCrop = () => {
          ...
          <GestureDetector gesture={composedGesture}>
            <Animated.View style={[styles.imageContainer, animatedStyle]}>
              <ExpoImage
                source={{ uri }}
                onLoad={() => {
                  // Resetea las transformaciones cuando la nueva imagen recortada se haya cargado
                  scale.value = 1;
                  translateX.value = 0;
                  translateY.value = 0;
                }}
