...
export default function UserProfile({ route, navigation }) {
    ...
    useEffect(() => {
        ...
        fetchUserData({ selectedUser, setIsPrivate, user, navigation, setPhotoUrls, setFirstHobby, setSecondHobby, setRelationshipStatus, setFirstInterest, setSecondInterest });
        fetchFriendCount({ selectedUser, setFriendCount });
        fetchEvents({ selectedUser, blockedUsers, setEvents, parseEventDate });
        checkFriendship({ user, selectedUser, setFriendshipStatus, setPendingRequest });
                                                        ...
                                                        <View style={styles.ovalAndIconsContainer}>
                                                            ...
                                                            <View style={styles.iconsContainer}>
                                                                <TouchableOpacity
                                                                    ...
                                                                    onPress={() => toggleUserStatus({user, selectedUser, setPendingRequest, setFriendshipStatus, setFriendCount, friendCount, t})}
                                                                >
                                                                    <AntDesign
                                                                        name={
                                                                            friendshipStatus
                                                                ...
                                                                <TouchableOpacity
                                                                    style={styles.iconButton}
                                                                    onPress={() => handleLikeProfile({ blockedUsers, selectedUser, isLiked, t, likeCount, setIsLiked, setLikeCount, user })}
                                                                >
                                                                    <AntDesign
                                                                ...
                                                                <TouchableOpacity
                                                                    style={styles.iconButton}
                                                                    onPress={() => handleSendMessage({ blockedUsers, selectedUser, navigation, t })}
                                                                >
                                                                    <AntDesign
                                                                        name="message1"
                                                                        size={24}
                    ...
                    {isElementsVisible && (
                        <View style={styles.menuContainer}>
                            ...
                            >
                                <Menu.Item
                                    onPress={() => {
                                        if (!blockedUsers.includes(selectedUser.id)) {
                                            toggleUserStatus({user, selectedUser, setPendingRequest, setFriendshipStatus, setFriendCount, friendCount, t});
                                        } else {
                                            Alert.alert(
                                ...
                                />
                                <Menu.Item
                                    onPress={() => handleReport({ selectedUser, setIsReportModalVisible, setMenuVisible, t })}
                                    title={t("userProfile.report")}
                                />
                                <Menu.Item
                                    onPress={() => handleToggleHiddenStories({ user, selectedUser, t, setHideStories, hideStories })}
                                    title={
                                        hideStories
                                ...
                                />
                                <Menu.Item
                                    onPress={() => toggleHideMyStories({user, selectedUser, t, setHideMyStories, hideMyStories})}
                                    title={
                                        hideMyStories
